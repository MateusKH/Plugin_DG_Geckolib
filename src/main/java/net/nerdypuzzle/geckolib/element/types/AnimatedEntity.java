...
public class AnimatedEntity extends GeneratableElement
    ...
    public BufferedImage generateModElementPicture() {
        Texture texture = Texture.fromName(getModElement().getWorkspace(), TextureType.ENTITY,
                FilenameUtilsPatched.removeExtension(mobModelTexture));
        if (texture == null)
            return MinecraftImageGenerator.Preview.generateMobPreviewPicture(null, spawnEggBaseColor, spawnEggDotColor, hasSpawnEgg);

        return MinecraftImageGenerator.Preview.generateMobPreviewPicture(
                texture.getTextureIcon(getModElement().getWorkspace()).getImage(),
                spawnEggBaseColor, spawnEggDotColor, hasSpawnEgg);
    }

    public boolean hasDrop() {
        return mobDrop != null && !mobDrop.isEmpty();
    }

    public boolean hasCustomProjectile() {
        return ranged && "Default item".equals(rangedItemType) && rangedAttackItem != null && !rangedAttackItem.isEmpty();
    }
